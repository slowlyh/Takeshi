...
export async function handler(chatUpdate) {
	...
	try {
		...
		if (
			!global.db.data.settings[this.user.jid].public &&
			!isMods &&
			!isOwner &&
			!m.fromMe
		)
		...
		const user =
			(m.isGroup
				? participants.find((u) => conn.getJid(u.id) === m.sender)
				: {}) || {}; // User Data
		const bot =
			(m.isGroup
				? participants.find((u) => conn.getJid(u.id) == this.user.jid)
				: {}) || {}; // Your Data
...
global.dfail = (type, m, conn) => {
	let msg = {
		...
		botAdmin:
			"Only Bot Admin - Command ini hanya bisa digunakan ketika bot menjadi admin",
