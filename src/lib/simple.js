	...
	let sock = Object.defineProperties(conn, {
			...
			async value(jid, type = "preview", timeoutMs) {
				try {
					...
					return fs.readFileSync(
						path.resolve(__dirname, "../media/avatar_contact.png")
					);
		...
		sendMedia: {
			...
			async value(jid, path, quoted, options = {}) {
				...
				let pase =
					messageType.replace("application", "document") ||
					messageType;
...
END:VCARD
		...
		cMod: {
			...
			value(
				...
				copy.key.fromMe =
					areJidsSameUser(sender, conn.user.id) || false;
		...
		downloadM: {
			...
			async value(m, type, saveToFile) {
				...
				return saveToFile && fs.existsSync(filename)
					? filename
					: buffer;
			},
			enumerable: true,
		...
		parseMention: {
			 ...
			 */
			value(text = "") {
				const raw = [...text.matchAll(/@([0-9]{5,16}|0)/g)].map(
					(v) => v[1] + "@lid"
				);
				return raw.map((jid) => conn.getJid(jid));
			},
			enumerable: true,
		},
		getName: {
				...
				return Object.entries(conn.chats)
					...
					.find(([_, { messages }]) =>
						Object.entries(messages).find(
							([k, v]) =>
								k === messageID || v.key?.id === messageID
						)
...
export function serialize() {
	...
	return Object.defineProperties(proto.WebMessageInfo.prototype, {
		...
		fromMe: {
			get() {
				return (
					areJidsSameUser(this.conn?.user.jid, this.sender) ||
					this.key?.fromMe ||
					false
				);
			},
			...
			get() {
				...
				return (
					...
					(type.length >= 3 &&
						...
						type[1]) || // Sometimes message in midle if mtype length is greater than or equal to 3
					type[type.length - 1]
				); // common case
			},
			enumerable: true,
			...
			get() {
				...
				return Object.defineProperties(
					...
					),
					{
						...
						chat: {
							get() {
								return self.conn.getJid(
									contextInfo.remoteJid || self.chat
								);
							},
							enumerable: true,
						},
						...
						sender: {
							get() {
								return self.conn.getJid(
									contextInfo.participant || this.chat
								);
							},
							enumerable: true,
						},
