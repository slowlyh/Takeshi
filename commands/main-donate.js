...
let handler = async (m, { text }) => {
	...
	if (!global.pakasir || !pakasir.slug || !pakasir.apikey)
		return m.reply("`pakasir.slug` dan `pakasir.apikey` belum di isi.");
	...
	expiredAt.setMinutes(
		expiredAt.getMinutes() + (global.pakasir.expired || 1)
	);
	...
	const sQris = await conn.sendMessage(
		...
		{
			image: {
				url: `https://quickchart.io/qr?text=${encodeURIComponent(cqris.payment_number)}`,
			},
			caption:
	...
	try {
		const res = await axios.post(
			"https://app.pakasir.com/api/transactioncreate/qris",
			{
				project,
				order_id:
					(global.namebot || "BOT").replace(/\s/g, "_") +
					"-" +
